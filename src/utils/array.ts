...
export const getArrayItemAndRemove = <T>(array: Array<T>, index: number): {
  item: T;
  items: Array<T>;
} => ({
  item: array[index],
...
export const getRandomArrayItemAndRemove = <T>(array: Array<T>) => getArrayItemAndRemove(array, getRandomArrayIndex(array))
...
export const getRandomRangeItemsAndRemove = <T>(array: Array<T>, range: number = 5) => {
  ...
  rangeEach(range, () => {
    const {
      item,
      items
    } = getRandomArrayItemAndRemove(filteredItems);
