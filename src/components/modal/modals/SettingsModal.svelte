...
<ModalWindow title={$t("modal.title.settings")} on:close={closeSettingsModal}>
  <NumberGroup
    ...
    min={0}
    max={4}
    bind:value={$settings.skipVertical}
  />
  <NumberGroup
    title={$t("modal.labels.skip_rows")}
    min={0}
    max={4}
    bind:value={$settings.skipHorizontal}
  />
  <NumberGroup
    title={$t("modal.labels.number_of_columns")}
    min={10}
    max={40}
    bind:value={$settings.numberOfColumns}
  />
  <NumberGroup
    title={$t("modal.labels.number_of_rows")}
    min={10}
    max={40}
    bind:value={$settings.numberOfRows}
  />
  <NumberGroup
    ...
    buttons={false}
    useMax={false}
    bind:value={$settings.wordLimit}
  />
  <TextGroup
    title={$t("modal.labels.custom_engpoint")}
    bind:value={$settings.endpoint}
    bind:toggle={$settings.useCustomEndpoint}
  />
</ModalWindow>
